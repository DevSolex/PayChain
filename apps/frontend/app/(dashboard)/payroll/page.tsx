...
const statusVariant: Record<string, 'success' | 'warning' | 'destructive' | 'secondary' | 'outline'> = {
  EXECUTED: 'success', PENDING: 'warning', FAILED: 'destructive', CANCELLED: 'secondary', APPROVED: 'outline',
}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => bulkMutation.mutate()} disabled={bulkMutation.isPending || pending === 0}>
            ...
            <TableBody>
              {isLoading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <TableRow key={i}>{Array.from({ length: 7 }).map((_, j) => <TableCell key={j}><div className="h-4 bg-muted rounded animate-pulse" /></TableCell>)}</TableRow>
                ))
              ) : payrolls.length === 0 ? (
                <TableRow>
                ...
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      {p.status === 'PENDING' && (
                        <Button variant="ghost" size="icon" className="h-7 w-7" title="Approve" onClick={() => approveMutation.mutate(p.id)}>
                          <CheckCircle className="w-3.5 h-3.5 text-green-500" />
                        </Button>
                      )}
                      {p.status === 'APPROVED' && (
                        <Button variant="ghost" size="icon" className="h-7 w-7" title="Execute" onClick={() => executeMutation.mutate(p.id)}>
                          <Play className="w-3.5 h-3.5 text-primary" />
                        </Button>
                      )}
                      {(p.status === 'PENDING' || p.status === 'APPROVED') && (
                        <Button variant="ghost" size="icon" className="h-7 w-7" title="Cancel" onClick={() => cancelMutation.mutate(p.id)}>
